...
from google_sheet import get_tasks_from_the_table, get_current_data_rows
...
def get_projects_todoist() -> dict:
    ...
    current_id_projects = {}
    current_id_projects['РЕХАУ'] = id_projects['РЕХАУ']
    current_id_projects['Промо'] = id_projects['Промо']
    return current_id_projects
        ...
        try:
            ...
            if title_task not in list_tasks:
                ...
                task = api.add_task(content=title_task, description=description, project_id=project_id,
                                    ...
                                    due_string=datetime.strptime(el[8], '%d.%m').strftime('%d %B'))
